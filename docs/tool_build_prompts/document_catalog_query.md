# Tool Build Prompt Archive — document_catalog_query
...
Deterministic metadata query tool for browsing and filtering TechVault’s ingested document catalog metadata stored in Postgres.

This tool is intended to provide stable, structured, non-semantic access to document catalog records for UI, admin, and agent/tool-layer catalog browsing.

It must not duplicate semantic search or RAG endpoints.
...
- Deterministic pagination is required
- Explicit tie-break ordering by `document_id` is required
- The implementation must remain read-only
- No semantic search, vector search, chunk retrieval, or answer generation belongs in this tool
...
The generated tool should satisfy the normal deterministic tool factory validation gates, including:

1. **Template conformance gate**
   - repository layout matches template conventions
   - required files exist
   - naming follows deterministic tool standards

2. **Manifest gate**
   - `tool.toml` exists
   - manifest fields match platform expectations
   - manifest naming and registration conventions are correct

3. **Schema/API gate**
   - request and response schemas are explicit
   - endpoint surface is minimal and stable
   - OpenAPI output is snapshot-tested

4. **Determinism gate**
   - explicit sorting everywhere
   - documented tie-break ordering via `document_id`
   - stable pagination under repeated identical queries
   - canonical JSON output where required

5. **Behavior gate**
   - filters work only on documented metadata fields
   - no semantic/vector/RAG behavior exists
   - read-only access only

6. **CLI gate**
   - CLI entrypoint exists
   - structured input/output follows template
   - output is stable and machine-consumable
...
8. **Integration gate**
   - tool is ready for standard registration under TechVault platform files
   - no custom platform abstractions were introduced

---

## Notes

This archive entry documents the intended generation path for `document_catalog_query` as a small, composable, deterministic metadata query primitive rather than a general search or retrieval system.

Its design boundary is deliberate: catalog metadata browsing only.